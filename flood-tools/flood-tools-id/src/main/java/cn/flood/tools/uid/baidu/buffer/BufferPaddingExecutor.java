...
/**
 * Represents an executor for padding {@link RingBuffer}<br> There are two kinds of executors: one
 * for scheduled padding, the other for padding immediately.
 *
 * @author yutianbao
 */
public class BufferPaddingExecutor {
  ...
  public BufferPaddingExecutor(RingBuffer ringBuffer, BufferedUidProvider uidProvider,
    ...
    bufferPadExecutors = new ThreadPoolExecutor(cores * 2, cores * 2, 0L, TimeUnit.MILLISECONDS,
        new LinkedBlockingQueue<Runnable>(), new NamingThreadFactory(WORKER_NAME));

    // initialize schedule thread
    if (usingSchedule) {
      bufferPadSchedule = new ScheduledThreadPoolExecutor(1,
          new NamingThreadFactory(SCHEDULE_NAME, true));
    } else {
