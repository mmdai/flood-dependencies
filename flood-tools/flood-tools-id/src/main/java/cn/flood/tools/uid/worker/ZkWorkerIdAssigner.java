  ...
  public long action() {
    try {
      ...
      if (null == workerId || null != zkClient.exists(workNode, false)) {
        ...
        // b、 不存在，创建ip:port节点
        if (null == workerId) {
          String nodePath = zkClient
              .create(workNode, new byte[0], Ids.OPEN_ACL_UNSAFE, CreateMode.PERSISTENT_SEQUENTIAL);
          workerId = Long.valueOf(nodePath.substring(nodePath.length() - 10));
        }
      }
      ...
      temporaryNode = zkClient
          .create(UID_TEMPORARY + ZK_SPLIT + workerId, longToBytes(System.currentTimeMillis()),
      ...
      for (String itemNode : activeNodes) {
        Long nodeTime = bytesToLong(
            zkClient.getData(UID_TEMPORARY + ZK_SPLIT + itemNode, false, new Stat()));
        sumTime += nodeTime;
      }
