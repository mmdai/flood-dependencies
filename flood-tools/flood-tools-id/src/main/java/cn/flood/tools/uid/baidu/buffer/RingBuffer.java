...
import cn.flood.tools.uid.baidu.utils.PaddedAtomicLong;
...
/**
 * Represents a ring buffer based on array.<br> Using array could improve read element performance
 * due to the CUP cache line. To prevent the side effect of False Sharing, {@link PaddedAtomicLong}
...
public class RingBuffer {
   ...
   *                      threshold, it will trigger padding buffer<br> Sample: paddingFactor=20,
   *                      bufferSize=1000 -> threshold=1000 * 20 /100, padding buffer will be
  ...
  /**
   * Put an UID in the ring & tail moved<br> We use 'synchronized' to guarantee the UID fill in slot
   * & publish new tail sequence as atomic operations<br>
   *
   * <b>Note that: </b> It is recommended to put UID in a serialize way, cause we once batch
   * generate a series UIDs and put the one by one into the buffer, so it is unnecessary put in
   * multi-threads
   *
   * @param uid
  ...
  public long take() {
    ...
    if (currentTail - nextCursor < paddingThreshold) {
      LOGGER.info("Reach the padding threshold:{}. tail:{}, cursor:{}, rest:{}", paddingThreshold,
          currentTail,
