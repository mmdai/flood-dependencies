package cn.flood.core.standard.distributed.lock;
...
public class EtcdSharedLock implements ISharedLock {
   ...
   * 整锁命令
   */
  private static final List<String> LOCK_APPLY = Arrays
      .asList(new String[]{"-XPUT", "-d", "value=1", "-d", "prevExist=false", "-d"});
  /**
   * 续租命令
   */
  private static final List<String> LOCK_RELET = Arrays
      .asList(new String[]{"-XPUT", "-d", "refresh=true", "-d", "prevExist=true", "-d"});
  /**
   * 解锁命令
   */
  ...
  public void startHeartBeatThread() {
    scheduledpool = new ScheduledThreadPoolExecutor(1,
        new NamingThreadFactory(Thread.currentThread().getName().concat(LOCK_HEART_BEAT), true));
