...
public final class ThreadPoolFactory {
  ...
  public static void main(String[] args) {
    ExecutorService executorService = new ThreadPoolFactory()
        .getCustomThreadPool(2, 5, 5L, new LinkedBlockingQueue<Runnable>());
    for (int j = 0; j < 10; j++) {
      executorService.submit(new Runnable() {
        ...
        public void run() {
          ...
          System.out
              .println(name + " " + i + " 结束,花费 " + (System.currentTimeMillis() - start) + "s");
        }
      });
    }
  }
  ...
  public ExecutorService getSimpleThreadPool(int corePoolSize, int maximumPoolSize) {
    return getCustomThreadPool(corePoolSize, maximumPoolSize, KEEP_ALIVE_TIME,
  ...
  public ExecutorService getCustomThreadPool(int corePoolSize,
      ...
      Long keepAliveTime,
      BlockingQueue<Runnable> blockingQueue) {
    logger.info("开始初始化线程池[corePoolSize={},maximumPoolSize={},keepAliveTime={}s]...", corePoolSize,
   ...
   * @param <V>
   */
  public <V> void awaitTasksFinished(List<Future> futureList,
