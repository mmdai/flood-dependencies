...
public class OrikaProperties {
  ...
  /**
   * Whether to use auto-mapping (MapperFactoryBuilder#useAutoMapping(boolean)). Follows Orika's
   * behavior by default.
   */
  private Boolean useAutoMapping;

  /**
   * Whether to map null values (MapperFactoryBuilder#mapNulls(boolean)). Follows Orika's behavior
   * by default.
   */
  private Boolean mapNulls;

  /**
   * Whether to dump the current state of the mapping infrastructure objects upon occurrence of an
   * exception while mapping (MapperFactoryBuilder#dumpStateOnException(boolean)). Follows Orika's
   * behavior by default.
   */
