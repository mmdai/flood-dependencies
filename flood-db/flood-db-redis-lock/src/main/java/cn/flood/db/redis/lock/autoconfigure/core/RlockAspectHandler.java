...
/**
 * Created by on 2017/12/29. Content :给添加@RLock切面加锁处理
 */
@Aspect
@Component
@Order(0)
public class RlockAspectHandler {
  ...
  private Object handleCustomLockTimeout(String lockTimeoutHandler, JoinPoint joinPoint)
    ...
    try {
      ...
      throw new RlockInvocationException(
  ...
  private void releaseLock(Rlock rlock, JoinPoint joinPoint, String currentLock) throws Throwable {
    ...
    if (Objects.isNull(lockRes)) {
      throw new NullPointerException(
          "Please check whether the input parameter used as the lock key value has been modified in the method, which will cause the acquire and release locks to have different key values and throw null pointers.currentLockKey:"
              + currentLock);
    }
    if (lockRes.getRes()) {
   ...
   * 处理释放锁时已超时
   */
  private void handleReleaseTimeout(Rlock rlock, LockInfo lockInfo, JoinPoint joinPoint)
      throws Throwable {
