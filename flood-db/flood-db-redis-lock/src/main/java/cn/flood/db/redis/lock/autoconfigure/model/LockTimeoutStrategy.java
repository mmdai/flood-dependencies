...
import cn.flood.db.redis.lock.autoconfigure.handler.lock.LockTimeoutHandler;
...
public enum LockTimeoutStrategy implements LockTimeoutHandler {
  ...
  FAIL_FAST() {
    @Override
    public void handle(LockInfo lockInfo, Lock lock, JoinPoint joinPoint)
        throws RlockTimeoutException {

      String errorMsg = String
          .format("Failed to acquire Lock(%s) with timeout(%ds)", lockInfo.getName(),
      ...
      while (!lock.acquire()) {
        ...
        if (interval > DEFAULT_MAX_INTERVAL) {
          String errorMsg = String.format(
              "Failed to acquire Lock(%s) after too many times, this may because dead lock occurs.",
              lockInfo.getName());
          throw new RlockTimeoutException(errorMsg);
        }
