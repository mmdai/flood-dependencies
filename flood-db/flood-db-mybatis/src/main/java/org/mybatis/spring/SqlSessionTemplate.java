...
public class SqlSessionTemplate implements SqlSession, DisposableBean {
  ...
  private class SqlSessionInterceptor implements InvocationHandler {
    ...
    public Object invoke(Object proxy, Method method, Object[] args) throws Throwable {
      ...
      if (!BatchInsertUtil.isBatchInsertMethod(mapperFullMethodName)) {
        return originalInvoke(proxy, method, args);
      } else {
        return doBatchInsert(proxy, method, args);
      }
    }

    /**
     * Mybatis-spring原有的invoke代码
     */
    private Object originalInvoke(Object proxy, Method method, Object[] args) throws Throwable {
    ...
    private int doBatchInsert(Object proxy, Method method, Object[] args) throws Throwable {
      ...
      // 实体列表，这样强制转换为List，是因为和业务开发者约定了@BatchInsert注解的方法参数必须为List类型
      List<Object> toBeInsertObjects = (List<Object>) args[1];

      // 将args[1]对应的要插入的多个实体，分多次插入
      // 复用，不用每次都去new
      Object[] newArgs = new Object[2];
      newArgs[0] = mapperFullMethodName;

      try {
        for (Object object : toBeInsertObjects) {
          newArgs[1] = object;
          Object result = method.invoke(sqlSession, newArgs);
        }
