package cn.flood.db.mybatis.plus.constant;
...
public enum Template {
  ...
  BATCH_ADD("batchAdd", "批量添加数据", "<script>INSERT INTO %s %s VALUES %s</script>"),
  UPDATE("update", "增量更新数据", "<script>UPDATE %s %s WHERE %s=#{%s}</script>"),
  BATCH_UPDATE("batchUpdate", "批量增量更新数据", "<script>UPDATE %s %s WHERE %s=#{%s}</script>"),
  ...
  GET_BY_CONDITIONS("getByConditions", "根据Conditions获取一个对象",
      "<script>SELECT %s FROM %s %s LIMIT 0,1</script>"),
  ...
  GET_COUNT_BY_CONDITIONS("getCountByConditions", "根据条件获取总记录数",
  ...
  SEARCH_BY_CONDITIONS("searchByConditions", "根据Conditions 查询数据",
