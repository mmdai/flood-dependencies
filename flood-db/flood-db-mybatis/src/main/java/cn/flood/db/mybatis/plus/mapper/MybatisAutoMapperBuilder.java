...
import cn.flood.db.mybatis.plus.constant.Template;
import cn.flood.db.mybatis.plus.languagedriver.ConditionsLanguageDriver;
import cn.flood.db.mybatis.plus.support.BaseMapper;
...
public class MybatisAutoMapperBuilder {
  ...
  public MybatisAutoMapperBuilder(MybatisConfiguration mybatisConfiguration) {
    ...
    this.conditionsLanguageDriver = mybatisConfiguration.getLanguageRegistry()
        .getDriver(ConditionsLanguageDriver.class);
  }
  ...
  public void inject(MapperBuilderAssistant builderAssistant, Class boundType) {
    ...
    if (null != actualModelClass) {
      ...
      if (null != table.getIdTableField()) {
        ...
        batchUpdate(builderAssistant, boundType, actualModelClass, table);
      ...
      batchAdd(builderAssistant, boundType, actualModelClass, table);
    }
  }
  ...
  private MappedStatement addMappedStatement(MapperBuilderAssistant builderAssistant,
      Class<?> mapperClass, String id, SqlSource sqlSource,
      SqlCommandType sqlCommandType, Class<?> parameterClass, String resultMap, Class<?> resultType,
      KeyGenerator keyGenerator, String keyProperty, String keyColumn) {
    return addMappedStatement(builderAssistant, mapperClass, id, sqlSource, sqlCommandType,
        parameterClass,
  ...
  private MappedStatement addSelectMappedStatement(MapperBuilderAssistant builderAssistant,
      Class<?> mapperClass, String id, String sql, Class<?> resultType) {
    SqlSource sqlSource = languageDriver
        .createSqlSource(this.mybatisConfiguration, sql, resultType);
    /* 普通查询 */
    return this
        .addMappedStatement(builderAssistant, mapperClass, id, sqlSource, SqlCommandType.SELECT,
            null, null, resultType,
            new NoKeyGenerator(), null, null);
  }

  private MappedStatement addSelectConditionsMappedStatement(
      MapperBuilderAssistant builderAssistant, Class<?> mapperClass, String id, String sql,
      Class<?> resultType) {
    SqlSource sqlSource = this.conditionsLanguageDriver
        .createSqlSource(this.mybatisConfiguration, sql, resultType);
    /* 普通查询 */
    return this
        .addMappedStatement(builderAssistant, mapperClass, id, sqlSource, SqlCommandType.SELECT,
  ...
  private MappedStatement addInsertMappedStatement(MapperBuilderAssistant builderAssistant,
      Class<?> mapperClass,
      String id, String sql, Class<?> resultType, Table table) {
    SqlSource sqlSource = languageDriver
        .createSqlSource(this.mybatisConfiguration, sql, resultType);
    return this
        .addMappedStatement(builderAssistant, mapperClass, id, sqlSource, SqlCommandType.INSERT,
            resultType, null, int.class,
            new Jdbc3KeyGenerator(), table.getIdTableField().getColumnName(), null);

  }

  private MappedStatement addUpdateMappedStatement(MapperBuilderAssistant builderAssistant,
      Class<?> mapperClass, String id, String sql, Class<?> resultType) {
    SqlSource sqlSource = languageDriver
        .createSqlSource(this.mybatisConfiguration, sql, resultType);

    return this
        .addMappedStatement(builderAssistant, mapperClass, id, sqlSource, SqlCommandType.UPDATE,
            resultType, null, int.class,
            new NoKeyGenerator(), null, null);
  }

  private MappedStatement addDeleteMappedStatement(MapperBuilderAssistant builderAssistant,
      Class<?> mapperClass, String id, String sql, Class<?> resultType) {
    SqlSource sqlSource = languageDriver
        .createSqlSource(this.mybatisConfiguration, sql, resultType);
    return this
        .addMappedStatement(builderAssistant, mapperClass, id, sqlSource, SqlCommandType.DELETE,
            resultType, null, int.class,
            new NoKeyGenerator(), null, null);
  }

  private MappedStatement addDeleteConditionMappedStatement(MapperBuilderAssistant builderAssistant,
      Class<?> mapperClass, String id, String sql, Class<?> resultType) {
    SqlSource sqlSource = this.conditionsLanguageDriver
        .createSqlSource(this.mybatisConfiguration, sql, resultType);
    /* 普通查询 */
   ...
   * 根据id 查找数据
   */
  private void get(MapperBuilderAssistant builderAssistant, Class<?> mapperClass,
      Class<?> modelClass, Table table) {
    Template sqlMethod = Template.GET;
    // 组装语句
    String sql = String.format(sqlMethod.getSql(), table.getColumnsSqlAs(), table.getWrapName(),
        table.getIdTableField().getColumnName(), table.getIdTableField().getFieldName());
   ...
   * 查询所有数据
   */
  private void searchAll(MapperBuilderAssistant builderAssistant, Class<?> mapperClass,
      Class<?> modelClass, Table table) {
   ...
   * 查询总记录数
   */
  private void getCount(MapperBuilderAssistant builderAssistant, Class<?> mapperClass,
      Class<?> modelClass, Table table) {
   ...
   * 根据map 查询总记录数
   */
  private void getCountByMap(MapperBuilderAssistant builderAssistant, Class<?> mapperClass,
      Class<?> modelClass, Table table) {
    Template sqlMethod = Template.GET_COUNT_BY_MAP;
    String sql = String.format(sqlMethod.getSql(), table.getWrapName(), genWhereSqlByMap());

    addSelectConditionsMappedStatement(builderAssistant, mapperClass, sqlMethod.getMethod(), sql,
        Long.class);
  }
   ...
   * 根据map 查询总记录数
   */
  private void getCountByConditions(MapperBuilderAssistant builderAssistant, Class<?> mapperClass,
      Class<?> modelClass, Table table) {
   ...
   * 根据map 查询所有记录
   */
  private void getByMap(MapperBuilderAssistant builderAssistant, Class<?> mapperClass,
      Class<?> modelClass, Table table) {
   ...
   * 根据Conditions 查询所有记录
   */
  private void getByConditions(MapperBuilderAssistant builderAssistant, Class<?> mapperClass,
      Class<?> modelClass, Table table) {
    Template sqlMethod = Template.GET_BY_CONDITIONS;
    String sql = String.format(sqlMethod.getSql(), table.getColumnsSqlAs(), table.getWrapName(),
        genWhereSqlByCondition());
   ...
   * 根据map 查询所有记录
   */
  private void searchByMap(MapperBuilderAssistant builderAssistant, Class<?> mapperClass,
      Class<?> modelClass, Table table) {
    Template sqlMethod = Template.SEARCH_BY_MAP;
    String sql = String.format(sqlMethod.getSql(), table.getColumnsSqlAs(), table.getWrapName(),
        genWhereSqlByMap(), genOrderSqlByMap());
   ...
   * 根据map 查询所有记录
   */
  private void searchByConditions(MapperBuilderAssistant builderAssistant, Class<?> mapperClass,
      Class<?> modelClass, Table table) {
   ...
   * 添加数据
   */
  private void add(MapperBuilderAssistant builderAssistant, Class<?> mapperClass,
      Class<?> modelClass, Table table) {
    Template sqlMethod = Template.ADD;
    String sql = String.format(sqlMethod.getSql(), table.getWrapName(), table.getColumnsSql(),
        table.getFieldSql());
   ...
   * 批量添加数据
   */
  private void batchAdd(MapperBuilderAssistant builderAssistant, Class<?> mapperClass,
      Class<?> modelClass, Table table) {
    Template sqlMethod = Template.BATCH_ADD;
    String sql = String.format(sqlMethod.getSql(), table.getWrapName(), table.getColumnsSql(),
        table.getFieldSql());
   ...
   * 增量更新数据
   */
  private void update(MapperBuilderAssistant builderAssistant, Class<?> mapperClass,
      Class<?> modelClass, Table table) {
    Template sqlMethod = Template.UPDATE;
    String sql = String
        .format(sqlMethod.getSql(), table.getWrapName(), table.getNotNullUpdateSetSql(),
            table.getIdTableField().getColumnName(), table.getIdTableField().getFieldName());
   ...
   * 批量增量更新数据
   */
  private void batchUpdate(MapperBuilderAssistant builderAssistant, Class<?> mapperClass,
      Class<?> modelClass, Table table) {
    Template sqlMethod = Template.BATCH_UPDATE;
    String sql = String
        .format(sqlMethod.getSql(), table.getWrapName(), table.getNotNullUpdateSetSql(),
            table.getIdTableField().getColumnName(), table.getIdTableField().getFieldName());
   ...
   * 全量更新数据
   */
  private void updateAll(MapperBuilderAssistant builderAssistant, Class<?> mapperClass,
      Class<?> modelClass, Table table) {
    Template sqlMethod = Template.UPDATE_ALL;
    String sql = String.format(sqlMethod.getSql(), table.getWrapName(), table.getUpdateSetSql(),
        table.getIdTableField().getColumnName(), table.getIdTableField().getFieldName());
   ...
   * @param table
   */
  private void deleteByConditions(MapperBuilderAssistant builderAssistant, Class<?> mapperClass,
      Class<?> modelClass, Table table) {
    Template sqlMethod = Template.DELETE_BY_CONDITIONS;
    String sql = String
        .format(sqlMethod.getSql(), table.getWrapName(), genDeleteWhereSqlByCondition());
    addDeleteConditionMappedStatement(builderAssistant, mapperClass, sqlMethod.getMethod(), sql,
        modelClass);
  }
   ...
   * 删除数据
   */
  private void delete(MapperBuilderAssistant builderAssistant, Class<?> mapperClass,
      Class<?> modelClass, Table table) {
    Template sqlMethod = Template.DELETE;
    String sql = String
        .format(sqlMethod.getSql(), table.getWrapName(), table.getIdTableField().getColumnName(),
            table.getIdTableField().getFieldName());
