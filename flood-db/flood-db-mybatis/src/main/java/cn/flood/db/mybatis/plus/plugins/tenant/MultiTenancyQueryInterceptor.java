...
public class MultiTenancyQueryInterceptor implements Interceptor {
  ...
  public static void main(String[] args) {
    ...
    String finalSql = multiTenancyQueryInterceptor
        .appendWhereCondition(oderBySql.toLowerCase(), condition);
    ...
    finalSql = multiTenancyQueryInterceptor
        .appendWhereCondition(oderBySql.toLowerCase(), condition);
    System.out.println(flag);
    System.out.println(finalSql);
    ...
    oderBySql =
        " SELECT sc.platform_code, sc.shop_code,sc.stock_code,sc.operate_type, sc.change_amount,sc.order_num,sc.surplus_amount,sc.reason,sc.remarks,sc.ext, sc.updated_by,sc.update_time, st.spu_code, st.sku_code "
            +
  ...
  public Object intercept(Invocation invocation) throws Throwable {
    ...
    // 获取多租户查询条件
    Object queryObject;
    if (Objects.isNull(queryObject = this.getQueryObjectFromMultiTenancyQueryValueFactory(
        multiTenancy.multiTenancyQueryValueFactory()))) {
  ...
  private Object getQueryObjectFromMultiTenancyQueryValueFactory(
      Class<? extends MultiTenancyQueryValueFactory> multiTenancyQueryValueFactoryClass) {

    if (Objects.isNull(multiTenancyQueryValueFactoryClass)) {
      return null;
    }
    MultiTenancyQueryValueFactory multiTenancyQueryValueFactory = this.multiTenancyQueryValueFactoryMap
        .get(multiTenancyQueryValueFactoryClass);
    ...
    synchronized (this.multiTenancyQueryValueFactoryMap) {
      ...
      try {
        ...
        multiTenancyQueryValueFactoryMap
            .putIfAbsent(multiTenancyQueryValueFactoryClass, multiTenancyQueryValueFactory);
        return multiTenancyQueryValueFactory.buildMultiTenancyQueryValue();
      } catch (Exception e) {
        return null;
      }
  ...
  private int getConditionAndByIndex(String originSql, String condition, int conditionLength) {
    ...
    if (StringUtils.containsIgnoreCase(originSql, condition)
      ...
      while (-1 != (groupIndex = endOfGroupString.indexOf(condition))) {
        ...
        if (StringUtils.isBlank(endOfGroupString = endOfGroupString
            .substring(endOfGroupIndex, endOfGroupString.length()))) {
          break;
        }
  ...
  private int doGetConditionIndex(String originSql, int orderCount, String condtion,
      int conditionLength) {
  ...
  private boolean containConditionAndBy(String originSql, String condition, int conditionLength) {
    ...
    if (StringUtils.containsIgnoreCase(originSql, condition)
      ...
      while (-1 != (index = endOfString.indexOf(condition))) {
        ...
        if (StringUtils
            .isBlank(endOfString = endOfString.substring(endOfGroupIndex, endOfString.length()))) {
          break;
        }
  ...
  public void setProperties(Properties properties) {
    ...
    if (Objects.nonNull(multiTenancyQueryColumn = properties
        .get(MultiTenancyProperties.MULTI_TENANCY_QUERY_COLUMN_PROPERTY))) {
      multiTenancyProperties.setMultiTenancyQueryColumn(multiTenancyQueryColumn.toString());
    }
    Object conditionType;
    if (Objects
        .nonNull(conditionType = properties.get(MultiTenancyProperties.CONDITION_TYPE_PROPERTY))) {
