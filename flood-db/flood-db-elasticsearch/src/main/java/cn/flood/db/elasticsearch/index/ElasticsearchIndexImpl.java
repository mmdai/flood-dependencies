...
import cn.flood.db.elasticsearch.util.IndexTools;
...
public class ElasticsearchIndexImpl<T> implements ElasticsearchIndex<T> {
    ...
    if (metaData.isRollover()) {//如果配置了rollover则替换索引名称为rollover名称，并创建对应的alias
      if (metaData.getRolloverMaxIndexAgeCondition() == 0
        ...
        throw new RuntimeException(
            "rolloverMaxIndexAgeCondition is zero OR rolloverMaxIndexDocsCondition is zero OR rolloverMaxIndexSizeCondition is zero");
      }
  ...
  public MappingSetting getMappingSource(Class clazz, MetaData metaData) throws Exception {
    ...
    for (int i = 0; i < mappingDataList.length; i++) {
      ...
      //add date format
      if (DataType.date_type.toString().replaceAll("_type", "").equals(mappingData.getDatatype())
          && !CollectionUtils.isEmpty(mappingData.getDateFormat())) {
      ...
      if (!mappingData.getDatatype().equals(NESTED)) {
        if (mappingData.isNgram() &&
            ("text".equals(mappingData.getDatatype()) || "keyword"
                .equals(mappingData.getDatatype()))) {
          isNgram = true;
        }
        source.append(oneField(mappingData));
      } else {
        source.append(" ,\"properties\": { ");
        if (mappingData.getNested_class() != null
            && mappingData.getNested_class() != Object.class) {
          MappingData[] submappingDataList = indexTools
              .getMappingData(mappingData.getNested_class());
...
//                        source.append(oneField(mappingDataList[j]));
    ...
    if (isNgram) {
      builder = Settings.builder()
          ...
          .putList("analysis.analyzer.autocomplete.filter",
              new String[]{"lowercase", "autocomplete_filter"});
    } else {
      builder = Settings.builder()
          .put("index.number_of_shards", metaData.getNumber_of_shards())
          .put("index.number_of_replicas", metaData.getNumber_of_replicas())
    ...
    if (classPathResource.exists()) {
      List<String> settings = new BufferedReader(
  ...
  public void switchAliasWriteIndex(Class<T> clazz, String writeIndex) throws Exception {
    ...
    if (metaData.isAlias()) {//当配置了别名后自动创建索引功能将失效
      ...
      } else if (!Stream.of(metaData.getAliasIndex()).collect(Collectors.toList())
          .contains(metaData.getWriteIndex())) {
      ...
      logger.info(
          "更新Alias[" + metaData.getIndexname() + "]结果：" + indicesAliasesResponse.isAcknowledged());
    }
  }
  ...
  public void createAlias(Class<T> clazz) throws Exception {
    ...
    if (metaData.isAlias()) {//当配置了别名后自动创建索引功能将失效
      ...
      if (exists) {
        ...
        AcknowledgedResponse indicesAliasesResponse = client.indices()
            .updateAliases(request, RequestOptions.DEFAULT);
        logger.info("创建Alias[" + metaData.getIndexname() + "]结果：" + indicesAliasesResponse
            .isAcknowledged());
      }
    }
  }
  ...
  @Override
  public void createIndex(Map<String, String> settings, Map<String, String[]> settingsList,
      String mappingJson, String indexName) throws Exception {
  ...
  private String oneField(MappingData mappingData) {
    ...
    if (mappingData.isNgram() && ("text".equals(mappingData.getDatatype()) || "keyword"
        .equals(mappingData.getDatatype()))) {
    ...
    if (mappingData.isKeyword() && !"keyword".equals(mappingData.getDatatype()) && mappingData
        .isSuggest()) {
      source.append(" \n");
  ...
  private void rollover(MetaData metaData) throws Exception {
    ...
    if (metaData.getRolloverMaxIndexAgeCondition() != 0) {
      request.addMaxIndexAgeCondition(new TimeValue(metaData.getRolloverMaxIndexAgeCondition(),
          metaData.getRolloverMaxIndexAgeTimeUnit()));
    }
    ...
    if (metaData.getRolloverMaxIndexSizeCondition() != 0) {
      request.addMaxIndexSizeCondition(
          new ByteSizeValue(metaData.getRolloverMaxIndexSizeCondition(),
              metaData.getRolloverMaxIndexSizeByteSizeUnit()));
    }
    RolloverResponse rolloverResponse = client.indices().rollover(request, RequestOptions.DEFAULT);
    logger.info(
        "rollover alias[" + metaData.getIndexname() + "]结果：" + rolloverResponse.isAcknowledged());
  }
