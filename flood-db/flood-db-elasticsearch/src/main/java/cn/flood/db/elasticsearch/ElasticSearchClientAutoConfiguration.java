...
public class ElasticSearchClientAutoConfiguration {
  ...
  public RestHighLevelClient restHighLevelClient(ElasticsearchProperties elasticsearchProperties,
    ...
    for (int i = 0; i < hostNames.length; i++) {
      ...
      httpHostList[i] = new HttpHost(host[0].replaceAll("\\s*", ""),
          Integer.parseInt(host[1].replaceAll("\\s*", "")));
    }
     ...
     * 异步httpclient连接数配置
     */
    if (StringUtils.hasText(elasticsearchProperties.getUsername()) && StringUtils
        .hasText(elasticsearchProperties.getPassword())) {
