...
/**
 * program: esdemo description: 对应索引结构mapping的注解，在es entity field上添加 author: X-Pacific zhang create:
 * 2019-01-25 16:57
 **/
...
//    AnalyzedType analyzedtype() default AnalyzedType.analyzed;
  ...
  /**
   * null_value指定，默认空字符串不会为mapping添加null_value 对于值是null的进行处理，当值为null是按照注解指定的‘null_value’值进行查询可以查到
   * 需要注意的是要与根本没有某字段区分（没有某字段需要用Exists Query进行查询） 建议设置值为NULL_VALUE
   *
   * @return
   */
  ...
  /**
   * nested对应的类型，默认为Object.Class。 对于DataType是nested_type的类型才需要添加的注解，通过这个注解生成嵌套类型的索引 例如：
   *
   * @return
