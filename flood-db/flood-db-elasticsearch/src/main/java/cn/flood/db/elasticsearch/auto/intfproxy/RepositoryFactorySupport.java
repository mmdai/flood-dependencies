...
/**
 * program: esclientrhl description: 用于生成ESCRepository的代理bean author: X-Pacific zhang create:
 * 2019-09-02 23:09
 **/
public class RepositoryFactorySupport<T extends ESCRepository<S, ID>, S, ID> implements
    ApplicationContextAware, ResourceLoaderAware, InitializingBean, FactoryBean<T>,
  ...
  private void getMetadata(SimpleESCRepository target) throws Exception {
    ...
    if (cn.flood.db.elasticsearch.auto.util.EnableESTools.isPrintregmsg()) {
      logger.info("domainClassName：" + domainClassName + " idClassName：" + idClassName);
    }
    //按照实体类类型名称匹配实体类类型
    List<String> entityList = getEntityList();
    for (int i = 0; i < entityList.size(); i++) {
      if (entityList.get(i).lastIndexOf("." + domainClassName) != -1 || entityList.get(i)
          .equals(domainClassName)) {
