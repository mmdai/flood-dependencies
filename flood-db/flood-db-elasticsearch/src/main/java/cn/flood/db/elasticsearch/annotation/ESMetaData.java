...
/**
 * program: esdemo description: es索引元数据的注解，在es entity class上添加 create: 2019-01-18 16:12
 *
 * @author mmdai
 */
...
public @interface ESMetaData {
  ...
  /**
   * 检索时的索引名称，如果不配置则默认为和indexName一致，该注解项仅支持搜索 并不建议这么做，建议通过特定方法来做跨索引查询
   */
  ...
  /**
   * 当配置了alias后，指定哪个index为writeIndex 当前配置仅生效于配置了alias但没有配置rollover 注意：配置的index必须存在切在aliasIndex中
   *
   * @return
   */
