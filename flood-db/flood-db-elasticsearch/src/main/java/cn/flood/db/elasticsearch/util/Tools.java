  ...
  public static <T> List<List<T>> splitList(List<T> oriList, boolean isParallel) {
    ...
    if (isParallel) {
      return Stream.iterate(0, n -> n + 1).limit(limit).parallel().map(
          a -> oriList.stream().skip(a * Constant.BULK_COUNT).limit(Constant.BULK_COUNT).parallel()
              .collect(Collectors.toList())).collect(Collectors.toList());
    } else {
      final List<List<T>> splitList = new ArrayList<>();
      Stream.iterate(0, n -> n + 1).limit(limit).forEach(i -> {
        splitList.add(oriList.stream().skip(i * Constant.BULK_COUNT).limit(Constant.BULK_COUNT)
    ...
    if (checkNested.containsKey(obj.getClass())) {
      ...
      for (int i = 0; i < obj.getClass().getDeclaredFields().length; i++) {
        ...
        if (f.getAnnotation(ESMapping.class) != null
            ...
            || (f.getAnnotation(ESMapping.class).nested_class() != null
            && f.getAnnotation(ESMapping.class).nested_class() != Object.class))) {
