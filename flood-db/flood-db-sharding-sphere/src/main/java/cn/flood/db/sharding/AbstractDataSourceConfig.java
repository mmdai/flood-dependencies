...
public abstract class AbstractDataSourceConfig {
   ...
   * 根据配置构建数据源
   */
  public DataSource buildDataSource(MultiDataSourceRegister multiDataSourceConfig)
      throws SQLException {
    return CollectionUtils.isNotEmpty(multiDataSourceConfig.getTableRules()) ?
        buildShardingDataSource(multiDataSourceConfig)
        : multiDataSourceConfig.getDataSourceList().get(0);
  }
   ...
   * 构建shardingsphere数据源
   */
  private DataSource buildShardingDataSource(MultiDataSourceRegister multiDataSourceConfig)
      throws SQLException {
    //1.配置真实数据源
    Map<String, DataSource> dataSourceMap = buildDataSourceMap(
        multiDataSourceConfig.getDataSourceList());
    //2.配置分片规则
    ShardingRuleConfiguration shardingRuleConfig = buildShardingRuleConfiguration(
  ...
  private ShardingRuleConfiguration buildShardingRuleConfiguration(
      MultiDataSourceRegister multiDataSourceConfig) {
    //分片规则
    ...
    for (TableRuleProperties tableRule : multiDataSourceConfig.getTableRules()) {
      ...
      TableRuleConfiguration tableRuleConfiguration = new TableRuleConfiguration(
          tableRule.getLogicTable(),
          "ds${0.." + dbLastIndex + "}." + tableRule.getLogicTable() + "${0.." + tableLastIndex
              + "}");
      ;

      if (Func.isBlank(tableRule.getTableShardingAlgorithm())) {
        tableRuleConfiguration = new TableRuleConfiguration(tableRule.getLogicTable(),
            "ds${0.." + dbLastIndex + "}." + tableRule.getLogicTable());
      }
      //构建分库策略实例
      ...
      if (Func.isNotBlank(tableRule.getDbShardingAlgorithm())) {
        ...
        tableRuleConfiguration.setDatabaseShardingStrategyConfig(
            new ComplexShardingStrategyConfiguration(tableRule.getDbShardingColumns(),
                dbShardingAlgorithm));
      ...
      if (Func.isNotBlank(tableRule.getTableShardingAlgorithm())) {
        ...
        tableRuleConfiguration.setTableShardingStrategyConfig(
            new ComplexShardingStrategyConfiguration(tableRule.getTableShardingColumns(),
  ...
  private ComplexKeysShardingAlgorithm<Comparable<?>> buildAlgorithmInstance(
