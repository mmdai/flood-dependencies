...
public class RedisAutoConfiguration {
  ...
  /**
   * 提供给外部使用的RedisTemplater工厂类 用于获取动态RedisTemplater以及默认RedisTemplater
   *
   ...
   */
  @Bean
  public RedisTemplaterFactoryBuild redisTemplaterWrapperFactoryBuild(
      RedisConnectionFactory redisConnectionFactory, DynamicRedisProperties dynamicRedisProperties)
      throws Exception {
    RedisTemplaterFactoryBuild redisTemplaterFactoryBuild = getRedisTemplaterFactoryBuild(
        dynamicRedisProperties);
    redisTemplaterFactoryBuild.setDefaultRedisTemplate(
        redisTemplate(redisConnectionFactory, redisTemplaterFactoryBuild, dynamicRedisProperties));
   ...
   * @return
   */
  public RedisTemplaterFactoryBuild getRedisTemplaterFactoryBuild(
    ...
    if (dynamicRedisProperties.isEnabled()) {
      ...
      Map<String, RedisTemplate<String, Object>> stringRedisTemplateMap = redisTemplatesMap(
          yamlRedisProvider);
  ...
  /**
   * RedisTemplaterWrapper为RedisTemplater的包装类 当启用了多数据源配置时，RedisTemplaterWrapper中的数据源为配置中的其中一个
   * 1.当多数据源配置了默认数据源时，用默认数据源 2.当多数据源没有配置默认数据源时，使用最后一个配置作为数据源 当没有启用多数据源配置时，此类就是RedisTemplater的普通封装，配置什么数据源就用什么
   *
   * @return
   * @throws Exception
   */
  ...
  public RedisTemplate<String, Object> redisTemplate(RedisConnectionFactory redisConnectionFactory,
      RedisTemplaterFactoryBuild fb, DynamicRedisProperties drp) throws Exception {
