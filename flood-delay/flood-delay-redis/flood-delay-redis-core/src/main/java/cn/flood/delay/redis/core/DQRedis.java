	...
	public DQRedis(String ip, int port, String password, Duration timeout,
		...
		if (null != cluster && !CollectionUtils.isEmpty(cluster.getNodes())) {
			...
			for (String node : nodes) {
				...
				RedisURI.Builder uri = RedisURI.builder().withHost(parts[0]).withPort(Integer.parseInt(parts[1])).withTimeout(timeout);
			...
			RedisURI.Builder redisURI = RedisURI.builder().withHost(ip).withPort(port).withTimeout(timeout);
			if (!ObjectUtils.isEmpty(password)) {
