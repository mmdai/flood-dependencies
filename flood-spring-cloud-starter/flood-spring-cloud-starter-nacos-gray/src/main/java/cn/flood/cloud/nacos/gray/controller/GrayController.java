...
public class GrayController {
  ...
  public String deregisterInstance() {
    ...
    try {
      log.info(
          ...
          serviceName, groupName, clusterName, ip, port);
      nacosServiceManager.getNamingService()
          .deregisterInstance(serviceName, groupName, ip, port, clusterName);
  ...
  public String registerInstance() {
    ...
    int port = nacosDiscoveryProperties.getPort();
    try {
      log.info("register from nacos, serviceName:{}, groupName:{}, clusterName:{}, ip:{}, port:{}",
          serviceName, groupName, clusterName, ip, port);
      nacosServiceManager.getNamingService()
          .registerInstance(serviceName, groupName, ip, port, clusterName);
    } catch (NacosException e) {
