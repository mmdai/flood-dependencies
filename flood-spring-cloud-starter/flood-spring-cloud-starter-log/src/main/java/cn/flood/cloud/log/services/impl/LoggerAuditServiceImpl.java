...
public class LoggerAuditServiceImpl implements IAuditService {
  ...
  /**
   * 格式为：{时间}|{应用名}|{类名}|{方法名}|{用户id}|{用户名}|{租户id}|{操作类型}|{请求IP}|{主机IP}|{请求参数}|{操作信息} 例子：2020-02-04
   * 09:13:34.650|user-center|com.central.user.controller.SysUserController|saveOrUpdate|1|admin|webApp|OTHER|xxx.xxx.xxx.xxx|xxx.xxx.xxx.xxx|json|新增用户:admin
   */
  @Override
