...
public class WebsocketFilter implements GlobalFilter, Ordered {
  ...
  /**
   * @param exchange ServerWebExchange是一个HTTP请求-响应交互的契约。提供对HTTP请求和响应的访问， 并公开额外的 服务器
   *                 端处理相关属性和特性，如请求属性
   * @param chain
   * @return
   */
...
//        } else if ((requestUrl.getPath()).contains(webSocketProperties.getPath())) {
      ...
      if (Func.isNotEmpty(webSocketProperties.getPort())) {
        wsRequestUrl = UriComponentsBuilder.fromUri(requestUrl).scheme(wsScheme)
            .port(webSocketProperties.getPort()).build().toUri();
      }
