package cn.flood.cloud.grpc.sentinel.slot.flow;
...
public class FlowEarlyWarningSlot extends AbstractLinkedProcessorSlot<DefaultNode> {
  ...
  private FlowRule getOriginRule(String resource) {
    List<FlowRule> originRule = FlowRuleManager.getRules().stream()
        .filter(flowRule -> flowRule.getResource().equals(resource)).collect(Collectors.toList());
    ...
    if (rules != null) {
      for (FlowRule rule : rules) {
        ...
        if (!checker.canPassCheck(rule, context, node, count, prioritized)) {
          ...
          String originRuleCount =
              originRule == null ? "未知" : String.valueOf(originRule.getCount());
          log.info("FlowEarlyWarning: 服务{}目前的流量指标已经超过{}， 接近配置的流控阈值:{},", resource, rule.getCount(),
              originRuleCount);
          //TODO 报警功能自行实现
          break;
        }
