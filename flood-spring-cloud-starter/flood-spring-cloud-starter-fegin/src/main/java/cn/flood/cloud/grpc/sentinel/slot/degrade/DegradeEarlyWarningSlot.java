package cn.flood.cloud.grpc.sentinel.slot.degrade;
...
public class DegradeEarlyWarningSlot extends AbstractLinkedProcessorSlot<DefaultNode> {
  ...
  private List<DegradeRule> getRuleProvider(String resource) {
    ...
    return earlyWarningRuleList.stream().filter(rule -> resource.equals(rule.getResource()))
  ...
  private DegradeRule getOriginRule(String resource) {
    List<DegradeRule> originRule = DegradeRuleManager.getRules().stream()
        .filter(rule -> rule.getResource().equals(resource)).collect(Collectors.toList());
