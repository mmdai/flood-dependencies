...
public class RedisPersist implements IPersist {
  ...
  public boolean setConsumingIfNX(DedupElement dedupElement,
    ...
    String dedupKey = buildDedupMessageRedisKey(dedupElement.getApplication(),
        dedupElement.getTopic(), dedupElement.getTag(), dedupElement.getMsgUniqKey());

    //setnx, 成功就可以消费
    Boolean execute = redisTemplate.execute(
        (RedisCallback<Boolean>) redisConnection -> redisConnection
            .set(dedupKey.getBytes(), (CONSUME_STATUS_CONSUMING).getBytes(),
  ...
  public void markConsumed(DedupElement dedupElement, long dedupRecordReserveMinutes) {
    ...
    redisTemplate.opsForValue()
        .set(dedupKey, CONSUME_STATUS_CONSUMED, dedupRecordReserveMinutes, TimeUnit.MINUTES);
  ...
  public String toPrintInfo(DedupElement dedupElement) {
    return buildDedupMessageRedisKey(dedupElement.getApplication(), dedupElement.getTopic(),
        dedupElement.getTag(), dedupElement.getMsgUniqKey());
  }

  private String buildDedupMessageRedisKey(String applicationName, String topic, String tag,
      String msgUniqKey) {
    if (ObjectUtils.isEmpty(msgUniqKey)) {
      ...
      String prefix =
          "MSGDEDUP:" + applicationName + ":" + topic + (StringUtils.isNotEmpty(tag) ? ":" + tag
              : "");
