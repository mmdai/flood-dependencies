...
import cn.flood.mq.rocketmq.annotation.MQKey;
import cn.flood.mq.rocketmq.enums.DelayTimeLevel;
...
public class MessageBuilder {
  ...
  private static final String[] DELAY_ARRAY = "1s 5s 10s 30s 1m 2m 3m 4m 5m 6m 7m 8m 9m 10m 20m 30m 1h 2h"
      .split(" ");
          ...
          if (annotation.annotationType().equals(MQKey.class)) {
            ...
            for (Field field : fields) {
              ...
              if (field.getName().equals(mqKey.field())) {
                messageKey.append(
                    ObjectUtils.isEmpty(mqKey.prefix()) ? field.get(message).toString()
                        : (mqKey.prefix() + ":" + field.get(message).toString()));
                break;
              }
            }
            break;
