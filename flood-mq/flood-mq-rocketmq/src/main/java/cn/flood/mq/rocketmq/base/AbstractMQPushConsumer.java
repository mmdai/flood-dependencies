...
/**
 * Created by yipin on 2017/6/27. RocketMQ的消费者(Push模式)处理消息的接口
 */
public abstract class AbstractMQPushConsumer<T> extends AbstractMQConsumer<T> {
   ...
   * @return 处理结果
   */
  public ConsumeOrderlyStatus dealMessage(List<MessageExt> list,
   ...
   * @return 消费状态
   */
  public ConsumeConcurrentlyStatus dealDedupMessage(List<MessageExt> list,
      ConsumeConcurrentlyContext consumeConcurrentlyContext) {
    ...
    if (!hasConsumeFail) {//全都消费成功
      ...
      consumeConcurrentlyContext.setAckIndex(
          ackIndexIfFail);//标记成功位，后面的会重发以重新消费，在这个位置之前的不会重发。 详情见源码：ConsumeMessageConcurrentlyService#processConsumeResult
      LOGGER.warn("consume [{}] msg(s) fails, ackIndex = [{}] ", list.size(),
          consumeConcurrentlyContext.getAckIndex());
    }
  ...
  private boolean handleMsgInner(final MessageExt messageExt) {
    ...
    DedupElement dedupElement = new DedupElement(
        SpringBeanManager.getProperties("spring.application.name"), messageExt.getTopic(),
        messageExt.getTags() == null ? "" : messageExt.getTags(), dedupMessageKey(messageExt));
    ...
    if (shouldConsume != null && shouldConsume) {
      ...
      if (IPersist.CONSUME_STATUS_CONSUMING.equals(val)) {//正在消费中，稍后重试
        LOGGER.warn(
            "the same message is considered consuming, try consume later dedupKey : {}, {}, {}",
            persist.toPrintInfo(dedupElement), messageExt.getMsgId(),
            persist.getClass().getSimpleName());
        return false;
      } else if (IPersist.CONSUME_STATUS_CONSUMED.equals(val)) {//证明消费过了，直接消费认为成功
        LOGGER.warn("message has been consumed before! dedupKey : {}, msgId : {} , so just ack. {}",
            persist.toPrintInfo(dedupElement), messageExt.getMsgId(),
            persist.getClass().getSimpleName());
        return true;
      } else {//非法结果，降级，直接消费
        LOGGER.warn(
            "[NOTIFYME]unknown consume result {}, ignore dedup, continue consuming,  dedupKey : {}, {}, {} ",
            val, persist.toPrintInfo(dedupElement), messageExt.getMsgId(),
            persist.getClass().getSimpleName());
        return doHandleMsgAndUpdateStatus(messageExt, dedupElement, persist);
      }
    }
   ...
   */
  private boolean doHandleMsgAndUpdateStatus(final MessageExt messageExt,
      final DedupElement dedupElement, final IPersist persist) {
    // parse message body
    T t = parseMessage(messageExt);
    // parse ext properties
    Map<String, Object> ext = parseExtParam(messageExt);
    ...
    if (dedupElement.getMsgUniqKey() == null) {
      ...
      try {
        if (consumeRes) {//标记为这个消息消费过
          ...
          LOGGER.info("consume Res is false, try deleting dedup record {} , {}", dedupElement,
              persist);
