...
public class JDBCPersit implements IPersist {
  ...
  @Override
  public boolean setConsumingIfNX(DedupElement dedupElement,
      long dedupProcessingExpireMilliSeconds) {
    long expireTime = System.currentTimeMillis() + dedupProcessingExpireMilliSeconds;
    try {
      int i = jdbcTemplate.update(
          "INSERT INTO t_rocketmq_dedup(application_name, topic, tag, msg_uniq_key, status, expire_time) values (?, ?, ?, ?, ?, ?)",
          dedupElement.getApplication(), dedupElement.getTopic(), dedupElement.getTag(),
          dedupElement.getMsgUniqKey(), CONSUME_STATUS_CONSUMING, expireTime);
  ...
  private int delete(DedupElement dedupElement, boolean onlyExpire) {
    if (onlyExpire) {
      return jdbcTemplate.update(
          "DELETE FROM t_rocketmq_dedup  WHERE application_name = ? AND topic =? AND tag = ? AND msg_uniq_key = ? AND expire_time < ?",
          dedupElement.getApplication(), dedupElement.getTopic(), dedupElement.getTag(),
          dedupElement.getMsgUniqKey(), System.currentTimeMillis());
    } else {
      return jdbcTemplate.update(
          "DELETE FROM t_rocketmq_dedup  WHERE application_name = ? AND topic =? AND tag = ? AND msg_uniq_key = ?",
          dedupElement.getApplication(), dedupElement.getTopic(), dedupElement.getTag(),
          dedupElement.getMsgUniqKey());
    }
  }
  ...
  public void markConsumed(DedupElement dedupElement, long dedupRecordReserveMinutes) {
    ...
    int i = jdbcTemplate.update(
        "UPDATE t_rocketmq_dedup SET status = ? , expire_time  = ? WHERE application_name = ? AND topic = ? AND tag = ? AND msg_uniq_key = ? ",
        CONSUME_STATUS_CONSUMED, expireTime, dedupElement.getApplication(), dedupElement.getTopic(),
        dedupElement.getTag(), dedupElement.getMsgUniqKey());
  }

  @Override
  public String get(DedupElement dedupElement) {
    Map<String, Object> res = jdbcTemplate.queryForMap(
        "SELECT status FROM t_rocketmq_dedup where application_name = ? AND topic = ? AND tag = ? AND msg_uniq_key  = ? and expire_time > ?",
        dedupElement.getApplication(), dedupElement.getTopic(), dedupElement.getTag(),
        dedupElement.getMsgUniqKey(), System.currentTimeMillis());
