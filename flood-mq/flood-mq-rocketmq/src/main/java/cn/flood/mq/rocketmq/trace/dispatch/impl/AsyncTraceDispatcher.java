...
public class AsyncTraceDispatcher extends AsyncDispatcher {
  ...
  public AsyncTraceDispatcher(Properties properties) {
    int queueSize = Integer
        .parseInt(properties.getProperty(OnsTraceConstants.AsyncBufferSize, "2048"));
     ...
     */
    this.notifyThreshold = Integer
        .parseInt(properties.getProperty(OnsTraceConstants.WakeUpNum, "1"));
  ...
  public boolean append(Object ctx) {
    ...
    for (; ; ) {
      ...
      if (size >= qsize) {
        CLIENT_LOG.info(
            "msgtrace buffer is full,the loss count is" + discardCount.incrementAndGet() + "  "
                + ctx);
        return false;
      }
