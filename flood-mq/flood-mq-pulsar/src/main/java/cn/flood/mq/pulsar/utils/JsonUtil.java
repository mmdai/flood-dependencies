...
import cn.flood.mq.pulsar.utils.json.LocalDateSerializer;
import cn.flood.mq.pulsar.utils.json.LocalDateTimeSerializer;
...
public class JsonUtil {
  ...
  public static <T> T fromStr(String json, TypeReference<?> valueTypeRef) {
    ...
    try {
      ...
      log.error("TypeReference: {} deserialize from Json {} failed: {}", valueTypeRef, json,
          ex.getMessage(), ex);
    }
    return deSerialObj;
  }
  ...
  public static <T> T fromStr(String json, JavaType javaType) {
    ...
    try {
      ...
      log.error("JavaType: {} deserialize from Json {} failed: {}", javaType, json, ex.getMessage(),
          ex);
    }
    return deSerialObj;
  }
  ...
  public static <T> T fromStrByPath(String json, String path, TypeReference<?> valueTypeRef) {
    ...
    try {
      ...
      log.error("json from string",
          "TypeReference: {} deserialize from Json: {} failed: {}", valueTypeRef, json,
          e.getMessage());
    }
    return deSerialObj;
  }
  ...
  private static JsonNode getNode(JsonNode jsonNode, String nodeName) {
    ...
    if (nodeName.contains(LEFT_BRACKET)) {
      ...
      index = Integer.parseInt(
          nodeName.substring(nodeName.indexOf(LEFT_BRACKET) + 1, nodeName.indexOf(RIGHT_BRACKET)));
    } else {
      key = nodeName;
    }
  ...
  public static <K, V> Map<K, V> getMapFromStr(String json, Class<K> keyClass,
      Class<V> valueClass) {
    ...
    try {
      ...
      log.error("Collection of class:{} deSerial from json:{} has error:{}", keyClass, json,
          e.getMessage());
    }
    return deSerialList;
  }
  ...
  public static String textField(JsonNode node, String field, String defaultValue)
      throws Exception {
  ...
  public static boolean booleanField(JsonNode node, String field, boolean defaultValue)
      throws Exception {
    JsonNode jsonNode = node.get(field);
    if (jsonNode != null) {
      if (jsonNode.isNull()) {
