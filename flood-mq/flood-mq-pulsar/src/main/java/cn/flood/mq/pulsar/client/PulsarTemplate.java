...
public class PulsarTemplate {
  ...
  public class TemplateBuilder {
    ...
    /**
     * This identifies the type of topic. Pulsar supports two kind of topics: persistent and
     * non-persistent (persistent is the default, so if you don’t specify a type the topic will be
     * persistent). With persistent topics, all messages are durably persisted on disk (that means
     * on multiple disks unless the broker is standalone), whereas data for non-persistent topics
     * isn’t persisted to storage disks.
     */
    private boolean persistent = true;
    /**
     * The topic's tenant within the instance. Tenants are essential to multi-tenancy in Pulsar and
    ...
    public Map<String, MessageId> send(String message) throws PulsarClientException {
      ...
      if (ArrayUtils.isEmpty(topics)) {
        ...
        topicNameComponent
            .setTenancy(StringUtils.isNotEmpty(tenancy) ? tenancy : pulsarProperties.getTenancy());
        topicNameComponent.setNamespace(
            StringUtils.isNotEmpty(namespace) ? namespace : pulsarProperties.getNamespace());
        topicNameComponent.setPersistent(persistent);
        for (String topic : topics) {
          topic = PulsarUtils.getActualTopic(topicNameComponent, topic);
          TypedMessageBuilder<String> typeMessageBuilder = getTypeMessageBuilder(message, delay,
              timeUnit, topic);
     ...
     * @throws PulsarClientException pulsar send exception
     */
    public Map<String, CompletableFuture<MessageId>> sendAsync(String message)
      ...
      if (ArrayUtils.isEmpty(topics)) {
        ...
        TopicNameComponent topicNameComponent = new TopicNameComponent();
        topicNameComponent
            .setTenancy(StringUtils.isNotBlank(tenancy) ? tenancy : pulsarProperties.getTenancy());
        topicNameComponent.setNamespace(
            StringUtils.isNotBlank(namespace) ? namespace : pulsarProperties.getNamespace());
        topicNameComponent.setPersistent(persistent);
        for (String topic : topics) {
          topic = PulsarUtils.getActualTopic(topicNameComponent, topic);
          TypedMessageBuilder<String> typeMessageBuilder = getTypeMessageBuilder(message, delay,
