...
public class PulsarUtils {
  ...
  public static String getActualTopic(TopicNameComponent topicNameComponent, String value) {
    if (StringUtils
        .isAnyBlank(topicNameComponent.getTenancy(), topicNameComponent.getNamespace())) {
      throw new RuntimeException("[Pulsar] tenancy and namespace must be not blank");
    }
    return String.format("%s://%s/%s/%s",
        topicNameComponent.isPersistent() ? PulsarConstants.PERSISTENT
            : PulsarConstants.NON_PERSISTENT,
