 ...
 * @author mmdai
 */
public final class HttpRestLoggingInterceptor implements Interceptor {
  ...
  public HttpRestLoggingInterceptor() {
    this(Logger.DEFAULT);
  }

  public HttpRestLoggingInterceptor(Logger logger) {
    this.logger = logger;
  }
   ...
   */
  public HttpRestLoggingInterceptor setLevel(Level level) {
  ...
  public Response intercept(Chain chain) throws IOException {
    ...
    if (!logHeaders && hasRequestBody) {
      requestStartMessage.append(" ").append(" (").append(requestBody.contentLength())
          .append("-byte body");
    }
    requestStartMessage.append("\n");
    if (logHeaders) {
      if (hasRequestBody) {
        ...
        if (requestBody.contentType() != null) {
          requestStartMessage.append(" ").append("Content-Type: ").append(requestBody.contentType())
              .append("\n");
        }
        if (requestBody.contentLength() != -1) {
          requestStartMessage.append(" ").append("Content-Length: ")
      ...
      for (int i = 0, count = headers.size(); i < count; i++) {
        ...
        if (!"Content-Type".equalsIgnoreCase(name) && !"Content-Length".equalsIgnoreCase(name)) {
          requestStartMessage.append(" ").append(name).append(": ").append(headers.value(i))
              .append("\n");
        }
      }
    ...
    responseEndMessage.append("<=== ").append(response.code())
    ...
    if (logHeaders) {
      ...
      for (int i = 0, count = headers.size(); i < count; i++) {
        responseEndMessage.append(" ").append(headers.name(i)).append(": ").append(headers.value(i))
            .append("\n");
      }
