...
public class TokenUtil {
  ...
  public static UserToken buildToken(PlatformEnum platform, String tenantId, String subject,
    ...
    return buildToken(platform, tenantId, subject, expire, DEFAULT_EXPIRE_REFRESH_TOKEN, getKey(),
        needRt);
  }
   ...
   */
  public static UserToken buildToken(PlatformEnum platform, String tenantId, String subject,
      Long expire, Long rtExpire, Key key) {
   ...
   */
  public static UserToken buildToken(PlatformEnum platform, String tenantId, String subject,
      Long expire, Long rtExpire, Key key, boolean needRt) {
    tenantId = Func.toStr(tenantId, HeaderConstant.DEFAULT_TENANT_ID);
    String platformSubject =
        platform.getType() + StringPool.COLON + tenantId + StringPool.COLON + subject;
    Date expireDate = new Date(System.currentTimeMillis() + 1000 * expire);
    // 生成access_token
    String access_token = Jwts.builder().setSubject(platformSubject).signWith(key)
    ...
    if (needRt) {
      ...
      String refresh_token = Jwts.builder().setSubject(platformSubject).signWith(key)
          .setExpiration(refreshExpireDate).compact();
    ...
    if (token == null && tokenStore != null) {
      ...
      if (access_token != null && !access_token.trim().isEmpty()) {
        try {
          ...
          // 查询用户的角色和权限
          if (token != null) {
            token.setRoles(tokenStore
                .findRolesByUserId(PlatformEnum.valueOfEnum(type), tenantId, userId, token));
            token.setPermissions(tokenStore
                .findPermissionsByUserId(PlatformEnum.valueOfEnum(type), tenantId, userId, token));
          }
