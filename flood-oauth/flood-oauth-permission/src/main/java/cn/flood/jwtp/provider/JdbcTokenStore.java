...
public class JdbcTokenStore extends TokenStoreAbstract {
  ...
  private static final String BASE_SELECT =
      "select token_id, " + UPDATE_FIELDS + " from oauth_token";
  // 查询用户的某个token
  private static final String SQL_SELECT_BY_TOKEN = BASE_SELECT
      + " where user_id = ?  and platform = ? and tenant_id = ? order by create_time desc";
  // 查询某个用户的全部token
  private static final String SQL_SELECT_BY_USER_ID =
      BASE_SELECT + " where user_id = ? and platform = ? and tenant_id = ? order by create_time";
  // 插入token
  private static final String SQL_INSERT =
      "insert into oauth_token (" + UPDATE_FIELDS + ") values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
  // 删除某个用户指定token
  ...
  private static final String SQL_SELECT_REFRESH_TOKEN = BASE_SELECT
      + " where user_id = ? and platform = ?  and tenant_id = ? order by create_time desc";
  // 查询tokenKey
  ...
  public int storeToken(PlatformEnum platform, String tenantId, UserToken userToken) {
    ...
    // 限制用户的最大token数量
    if (getMaxToken() != -1) {
      List<UserToken> userUserTokens = findTokensByUserId(platform, tenantId,
          userToken.getUserId());
      if (userUserTokens.size() > getMaxToken()) {
        for (int i = 0; i < userUserTokens.size() - getMaxToken(); i++) {
          removeToken(platform, tenantId, userToken.getUserId(),
              userUserTokens.get(i).getAccessToken());
        }
      }
    }
    return rs;
  ...
  @Override
  public UserToken findToken(PlatformEnum platform, String tenantId, String userId,
      String access_token) {
    try {
      List<UserToken> list = jdbcTemplate
          .query(SQL_SELECT_BY_TOKEN, rowMapper, userId, platform.getType(), tenantId);
      if (CollectionUtil.isNotEmpty(list)) {
        return list.stream().filter(u -> access_token.equals(u.getAccessToken())).findFirst()
            .orElse(null);
      }
    } catch (EmptyResultDataAccessException e) {
    }
  ...
  public List<UserToken> findTokensByUserId(PlatformEnum platform, String tenantId, String userId) {
    try {
      return jdbcTemplate
          .query(SQL_SELECT_BY_USER_ID, rowMapper, userId, platform.getType(), tenantId);
    } catch (EmptyResultDataAccessException e) {
    }
    return null;
  }
  ...
  @Override
  public UserToken findRefreshToken(PlatformEnum platform, String tenantId, String userId,
      String refresh_token) {
    try {
      List<UserToken> list = jdbcTemplate
          .query(SQL_SELECT_REFRESH_TOKEN, rowMapper, userId, platform.getType(), tenantId);

      if (CollectionUtil.isNotEmpty(list)) {
        return list.stream().filter(u -> refresh_token.equals(u.getRefreshToken())).findFirst()
            .orElse(null);
      }
    } catch (EmptyResultDataAccessException e) {
    }
  ...
  @Override
  public int removeToken(PlatformEnum platform, String tenantId, String userId,
      String access_token) {
  ...
  @Override
  public int updateRolesByUserId(PlatformEnum platform, String tenantId, String userId,
      String[] roles) {
  ...
  @Override
  public int updatePermissionsByUserId(PlatformEnum platform, String tenantId, String userId,
      String[] permissions) {
  ...
  @Override
  public int updateUserInfoByUserId(PlatformEnum platform, String tenantId, String userId,
      String userInfo) {
    return jdbcTemplate.update(SQL_UPDATE_INFO, userInfo, userId, platform.getType(), tenantId);
  }

  @Override
  public String[] findRolesByUserId(PlatformEnum platform, String tenantId, String userId,
      UserToken userToken) {
    // 判断是否自定义查询
  ...
  @Override
  public String[] findPermissionsByUserId(PlatformEnum platform, String tenantId, String userId,
      UserToken userToken) {
    // 判断是否自定义查询
  ...
  private static class TokenRowMapper implements RowMapper<UserToken> {
    ...
    public UserToken mapRow(ResultSet rs, int rowNum) throws SQLException {
      ...
      Date refresh_token_expire_time = timestampToDate(
          rs.getTimestamp("refresh_token_expire_time"));
...
//            String platform = rs.getString("platform");
      ...
      userToken
          .setPermissions(StringUtils.stringListToArray(Func.parseList(permissions, String.class)));
