...
public class CheckPermissionUtil {
   ...
   * 检查权限是否符合
   */
  public static boolean checkPermission(UserToken userToken, HttpServletRequest request,
      HttpServletResponse response, Object handler, UrlPerm urlPerm) {
   ...
   * 检查角色是否符合
   */
  public static boolean checkRole(UserToken userToken, HttpServletRequest request,
      HttpServletResponse response, Object handler, UrlPerm urlPerm) {
    Method method = ((HandlerMethod) handler).getMethod();
   ...
   * 检查是否是没有权限或没有角色
   */
  public static boolean isNoPermission(UserToken userToken, HttpServletRequest request,
      HttpServletResponse response, Object handler, UrlPerm urlPerm) {
    return !checkPermission(userToken, request, response, handler, urlPerm) || !checkRole(userToken,
        request, response, handler, urlPerm);
  }
...
//                log.info("payload:【{}】", payload);
        if (!Func.isEmpty(payload)) {
          ...
          access_token = ((String) map.get(HeaderConstant.ACCESS_TOKEN))
              .replaceFirst(TokenUtil.PREFIX, StringPool.EMPTY);
        }
      }
    }
    return access_token;
