...
public class RedisTokenStore extends TokenStoreAbstract {
  ...
  public int storeToken(PlatformEnum platform, String tenantId, UserToken userToken) {
    ...
    if (userToken.getRefreshToken() != null
        && findRefreshToken(platform, tenantId, userToken.getUserId(), userToken.getRefreshToken())
        == null) {
      redisTemplate.opsForList()
          .rightPush(KEY_PRE_REFRESH_TOKEN + key, userToken.getRefreshToken());
      redisTemplate
          .expire(KEY_PRE_REFRESH_TOKEN + key, userToken.getRefreshTokenExpireSecond(), timeUnit);
    }
    // 存储角色
    ...
    // 存储权限
    updatePermissionsByUserId(platform, tenantId, userToken.getUserId(),
  ...
  public List<UserToken> findTokensByUserId(PlatformEnum platform, String tenantId, String userId) {
    ...
    if (accessTokens != null && accessTokens.size() > 0) {
      for (String accessToken : accessTokens) {
        ...
        // 查询权限
        userToken.setPermissions(
            StringUtils.stringSetToArray(redisTemplate.opsForSet().members(KEY_PRE_PERM + key)));
        // 查询角色
        userToken.setRoles(
            StringUtils.stringSetToArray(redisTemplate.opsForSet().members(KEY_PRE_ROLE + key)));
        // 查询信息
  ...
  public UserToken findRefreshToken(PlatformEnum platform, String tenantId, String userId,
    ...
    if (userId != null && !userId.trim().isEmpty() && refresh_token != null) {
      //key = 平台 + 租户 + 用户
      String key = getKey(platform, tenantId, userId);
      List<String> refreshTokens = redisTemplate.opsForList()
          .range(KEY_PRE_REFRESH_TOKEN + key, 0, -1);
  ...
  public String[] findRolesByUserId(PlatformEnum platform, String tenantId, String userId,
    ...
    if (jdbcTemplate != null) {
      try {
        List<String> roleList = ((org.springframework.jdbc.core.JdbcTemplate) jdbcTemplate)
            .query(getFindRolesSql(), new org.springframework.jdbc.core.RowMapper<String>() {
              @Override
  ...
  public String[] findPermissionsByUserId(PlatformEnum platform, String tenantId, String userId,
    ...
    if (jdbcTemplate != null) {
      try {
        List<String> permList = ((JdbcTemplate) jdbcTemplate)
            .query(getFindPermissionsSql(), new org.springframework.jdbc.core.RowMapper<String>() {
              @Override
              public String mapRow(ResultSet rs, int rowNum) throws SQLException {
                return rs.getString(1);
              }
  ...
  private String getKey(PlatformEnum platform, String tenantId, String userId) {
    return (platform.getType() + StringPool.COLON + Func
        .toStr(tenantId, HeaderConstant.DEFAULT_TENANT_ID) + StringPool.COLON + userId);
  }
